...
**우선순위는 "화면이 있으면 이긴다" 이지 "화면에만 넣을 수 있다" 가 아닙니다.**
화면에 등록이 없으면 `.env` 로 갑니다 (`choose_gemini`). 설치 중에는 `.env` 에
적는 쪽이 빠릅니다 — 화면 등록은 **관리자 계정이어야** 하는데, 가입 직후에는
일반 사용자라 `set_app_secret` 이 거부하기 때문입니다. 승격은 9-1 을 보세요.
...
- `GEMINI_API_KEY` — 웹 설정 화면에 **이미 등록했다면** 비워 둡니다. 아직이면
  여기에 적으세요 — 그대로 동작합니다 (5-4 참고)
...
| 증상 | 원인 |
|---|---|
| `Gemini 키 출처 — agent/.env` | **오류가 아닙니다.** `.env` 의 키를 쓴다는 안내입니다. 화면에 등록해 두고도 이게 나오면 그때는 등록이 안 물린 것입니다 |
| `Gemini API 키가 없습니다` | 양쪽 다 비어 있습니다. `.env` 에 적는 것이 가장 빠릅니다 |
| 메일 백엔드 ❌ | 폴더 이름이 다르거나, 아웃룩이 꺼져 있거나, 프로필 선택 창에서 멈춰 있습니다. **오류 끝에 그 위치의 실제 폴더 목록이 붙어 나옵니다** — 거기서 골라 `OUTLOOK_FOLDER` 에 적으세요 |
