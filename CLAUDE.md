...
- **주간 보고서의 기본 구간은 오늘이 속한 화~월입니다** (`weekOf`). 8/21(금)이면
  8/18(화)~8/24(월)이고, **아직 안 닫힌 구간**이므로 마감 전 중간 집계라는 사실을
  각주에 적습니다 — 안 적으면 확정 수치로 읽힙니다. 화면에서 `‹ ›` 로 다른 주를
  고를 수 있고, **오늘이 속한 구간보다 앞으로는 못 갑니다** (아직 아무 일도 안
  일어난 주라 빈 보고서가 나옵니다).
  구간을 만들려면 그 구간 안에 뜬 스냅샷이 있어야 합니다. 없으면 스냅샷이 속한
  구간까지만 만들 수 있고 화면이 배너로 밝힙니다 (`weekForSnapshot`) — 조용히 지난
  구간을 내놓으면 서식도 건수도 멀쩡해 보여서 아무도 못 알아챕니다. `doctor` 는
  스냅샷이 7일을 넘기면 실패로 셉니다.
...
- **주간보고서는 한 장입니다.** 이슈(3장)와 차주 계획(4장)은 진행 현황과 **같은
  장**에 있습니다 — 그게 이 보고서의 서식이고 받는 사람은 한 장을 봅니다. 표가
  넘치면 장을 늘리지 않고 행을 자르며, 몇 건 중 몇 건인지 **각주에 적습니다.**
  설정은 `build.ts` 의 `table.layouts`(base 하나) 와 `maxPages: 1` 이고 판정은
  `weekly.fitTable` 이 합니다.
  한때 표를 안 자르려고 3·4장을 압축하거나(`compact`) 다음 장으로 내렸는데
  (`spill`), 그것은 **서식을 바꾸는 일**이라 되돌렸습니다. 기계는 그 둘을 여전히
  할 줄 알지만 설정으로 막아 뒀습니다 — 서식을 바꾸는 것은 코드가 아니라 결정이고,
  결정이 바뀌면 그 한 곳만 고칩니다. **pptx 서식은 임의로 바꾸지 않습니다.**
