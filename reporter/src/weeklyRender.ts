...
import { defineSlideSize, hline, R, rect, round, sectionTitle as drawSectionTitle, text, type Slide } from './draw.ts'
import type { ProgressRow, WeeklyGroup, WeeklyModel, WeeklyRow } from './weekly.ts'
import { chipWidth, fitRail, type ProjectRail, type RailChip } from './milestones.ts'
...
export async function writeWeekly(m: WeeklyModel, nextLabel: string, outPath: string): Promise<void> {
